...
| Attaque AOE (orbe, façon Lux E) | Action **`aoe` = `Z`**, **en mode ATTACK**. 1ʳᵉ pression : `_launch_aoe_orb()` lance `AoeOrb.tscn` vers la cible souris (même `AimStrategy` que le tir). L'orbe voyage puis **détone** soit au **recast** (re-`Z`), soit à l'expiration de `aoe_fuse_time` (2 s). À la détonation, l'orbe fait `intersect_shape` (rayon `aoe_radius`, masque layer 2) → `take_hit(aoe_damage)` sur chaque ennemi, et `AoeBlast.tscn` montre l'onde. La requête tourne dans le pas physique de l'orbe. Cooldown de lancement `aoe_cooldown` (3 s) ; le recast est gratuit. |
