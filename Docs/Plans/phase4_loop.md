...
>   pour FOLLOW) porté par `CameraRig.get_aim_target()` (ex-`SphereCamera`,
>   renommé en phase 5 ; depuis la phase 5 le point visé est résolu par la
>   couche possession et transmis via `InputContext.world_cursor`) ;
...
| Visée souris | `CameraRig.get_aim_target(origin)` (ex-`SphereCamera`) délègue à une `AimStrategy` choisie selon le mode caméra : **topdown → `MouseCursorAim`** (curseur libre projeté), **follow → `ScreenCenterAim`** (souris capturée → centre écran). Raycast sur les ennemis, sinon plan du sol, sinon point lointain. *(Phase 5 : la couche possession appelle `get_aim_target` chaque frame et livre le résultat aux entités via `InputContext.world_cursor`.)* |
